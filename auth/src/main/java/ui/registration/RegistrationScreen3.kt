...
@Composable
fun RegistrationScreen3(navController: NavController, viewModel: RegisterViewModel = hiltViewModel()){
    AppTheme() {
            ...
            })
        {
            Column(
                ...
                TextButtonGroup(
                    ...
                    selectedIndex = state.difficulty,
                    onOptionSelected = { index ->
                        viewModel.onEvent(RegisterUIEvent.RegisterDifficultyChanged(index))
                    }
                )
                if (state.difficultyError != null) {
                    Text(
                        text = state.difficultyError,
                ...
                )
                {
                    ...
                    NFButton(text = stringResource(R.string.buttonRegisterScreen1), onButtonClicked = {viewModel.onEvent(RegisterUIEvent.NextClickedStep3)
                        val newState = viewModel.registerUIState
                        if (newState.difficultyError == null) {
                            navController.navigate("registrationScreen4") // ou registrationScreen3
                        }}
                    )
                }
